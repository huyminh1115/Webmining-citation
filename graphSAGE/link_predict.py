...
def compute_auc(pos_score, neg_score):
    ...
    precision = precision_score(torch.cat([torch.ones(pos_score.shape[0]), torch.zeros(neg_score.shape[0])]).numpy(), torch.cat([pos_score, neg_score]).numpy() > 0)
    recall = recall_score(torch.cat([torch.ones(pos_score.shape[0]), torch.zeros(neg_score.shape[0])]).numpy(), torch.cat([pos_score, neg_score]).numpy() > 0)
    accuracy = accuracy_score(torch.cat([torch.ones(pos_score.shape[0]), torch.zeros(neg_score.shape[0])]).numpy(), torch.cat([pos_score, neg_score]).numpy() > 0)
